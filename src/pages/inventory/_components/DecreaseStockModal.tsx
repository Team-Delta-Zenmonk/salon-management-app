...
import { decreaseStock } from "../../../features/inventory/decrease-items-stock/decrease-items-stock.service";
...
export const DecreaseStockModal: React.FC<DecreaseStockModalProps> = ({
  ...
  const onSubmit = async (data: any) => {
    ...
    setLoading(true);
    try {
      await decreaseStock(inventoryItem.uuid, inventoryItem.current_stock || 0, Number(data.quantity));
      callSnack("Stock decreased successfully", "success");
      onSuccess();
      onClose();
      reset();
    } catch (error: any) {
      callSnack(error?.response?.data?.message || "Failed to decrease stock", "error");
    } finally {
      setLoading(false);
    }
  };
