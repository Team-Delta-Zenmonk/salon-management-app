...
import { DecreaseStockModal } from "./DecreaseStockModal";
...
export const StockTable: React.FC<StockTableProps> = ({
  ...
  const StockProgressBar = ({ current, min }: { current: number; min?: number }) => {
    const isLow = min && min > 0 ? current <= min : current <= 0;
    const color = isLow ? theme.palette.error.main : theme.palette.success.main;
    const max = (min && min > 0) ? min * 2 : Math.max(current, 10);
    const percentage = Math.max(2, Math.min(100, (current / max) * 100));

    return (
      <Box sx={{ width: 60, height: 10, bgcolor: 'secondary.100', borderRadius: '8px', overflow: 'hidden', mr: 1 }}>
        <Box
          sx={{
            width: `${percentage}%`,
            height: '100%',
        ...
        {data.map((item) => {
          ...
          return (
            ...
            >
              ...
              <Box display="flex" alignItems="center" justifyContent="space-between" px={3} py={2} borderTop="1px solid" borderColor="divider" bgcolor="background.default">
                <Box>
