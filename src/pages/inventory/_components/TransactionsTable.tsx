...
export const TransactionsTable: React.FC<TransactionsTableProps> = ({
  ...
  const getStatus = (item: InventoryTransaction) => {
    if (item.received_date || item.received_quantity > 0) return "received";
    return "pending";
  };
              ...
              <TableBody>
                {data.map((item) => (
                  ...
                  >
                    ...
                    <TableCell align="right">
                      <Typography variant="paragraphMd">₹{parseFloat(item.bill_amount || "0").toFixed(2)}</Typography>
                    </TableCell>
                  </TableRow>
                ))}
                {data.length === 0 && (
