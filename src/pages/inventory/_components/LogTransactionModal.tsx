...
export const LogTransactionModal: React.FC<LogTransactionModalProps> = ({ open, onClose, onSuccess, transactionToEdit, createdItem, onAddNewItem }) => {
  ...
  const { control: itemControl, watch: watchItem, setValue: setItemValue } = useForm({
    defaultValues: {
      item_uuid: "",
    },
  });

  const itemUuid = watchItem("item_uuid");
  ...
  useEffect(() => {
    if (open) {
      if (transactionToEdit) {
        const itemObj = {
          label: `${transactionToEdit.item.name} - ${transactionToEdit.item.variant_name || 'Standard'}`,
          value: transactionToEdit.item.uuid,
          item_uuid: transactionToEdit.item.uuid,
        };
        setSelectedItem(itemObj);
        setItemValue("item_uuid", transactionToEdit.item.uuid);
        reset({
        ...
        const itemObj = {
          label: `${createdItem.name} - ${createdItem.variant_name || 'Standard'} (${createdItem.brand || 'No Brand'})`,
          value: createdItem.uuid,
          item_uuid: createdItem.uuid,
        };
        setSelectedItem(itemObj);
        setItemValue("item_uuid", createdItem.uuid);
        reset({
          ordered_date: "",
          received_date: "",
          ordered_quantity: 1,
  ...
  }, [open, reset, transactionToEdit, createdItem, setItemValue]);

  useEffect(() => {
    if (itemUuid) {
      const found = options.find((opt) => opt.item_uuid === itemUuid);
      if (found) {
  ...
  const fetchOptions = async (search: string) => {
    ...
    try {
      const res = await getStock({ search, limit: 50 });
      const flatItems = res.data.map((item: InventoryItem) => ({
  ...
  const onSubmit = async (data: any) => {
    if (!selectedItem) {
      callSnack("Please select an item", "error");
      return;
    }
    setLoading(true);
    try {
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <DialogActions sx={{ p: 2, px: 3, borderTop: "1px solid", borderColor: "divider" }}>
            ...
            <Button type="submit" variant="contained" disabled={loading || !selectedItem} sx={{ fontWeight: "bold", px: 3 }}>
