...
const AddItemStepperHeader = ({ activeStep }: { activeStep: number }) => {
  ...
  return (
    <div className="px-4 sm:px-6 py-3 sm:py-4 bg-muted/20 border-b border-border flex items-center justify-between gap-2 sm:gap-4 shrink-0">
      {steps.map((step, index) => {
        ...
        return (
          <div key={index} className="flex items-center gap-2 sm:gap-3 flex-1 last:flex-initial">
            <div className="flex items-center gap-2 sm:gap-3">
              <div
                className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center border transition-all duration-300 ${
                  isCompleted
              ...
              <div className="text-left">
                ...
                <p className="text-[10px] text-muted-foreground font-medium leading-none mt-1 hidden sm:block">
                  {step.description}
                </p>
              </div>
            </div>
            ...
            {index < steps.length - 1 && (
              <div className="flex-1 h-[2px] bg-border mx-2 sm:mx-4 hidden sm:block relative">
                <div
...
export const AddItemStepperModal: React.FC<AddItemStepperModalProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => !val && handleClose()}>
      <DialogContent className="w-[95vw] sm:max-w-4xl p-0 overflow-hidden rounded-2xl gap-0 flex flex-col max-h-[90vh] border border-border/80 shadow-2xl">
        <DialogHeader className="px-5 sm:px-6 py-4 border-b border-border shrink-0 bg-background/50 backdrop-blur-xs">
        ...
        <div className="flex-1 overflow-hidden relative bg-background flex flex-col min-h-0">
